	...
	public void bloodyBlow(Personnage opponent) {
		if (opponent.isBleeding) {
			if (isCriticalHit(1)) {
				...
				System.out.println("L'ennemi saigne ! " + opponent.getNameP() + " s'est pris 40 dégats ! Il lui reste "
						+ opponent.getHP() + " HP !");
			...
			if (isCriticalHit(0.2)) {
				...
				System.out.println("Coup critique ! " + opponent.getNameP() + " s'est pris 25 dégats ! Il lui reste "
						+ opponent.getHP() + " HP !");
				opponent.bleed(0.5);
			} else {
				opponent.setHP(opponent.getHP() - 15);
				System.out.println(
						opponent.getNameP() + " s'est pris 15 dégats ! Il lui reste " + opponent.getHP() + " HP !");
				opponent.bleed(0.3);
			}
		}
	}
